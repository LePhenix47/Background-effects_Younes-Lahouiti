import { NetworkParticlesCreator } from "../utils/classes/effects/network-effect-creator.class";
import {
  get2DContext,
  setCanvasSize,
...
const networkParticlesTemplateElement: HTMLTemplateElement =
  document.createElement("template");

const networkParticlesCssStyle: string = /* css */ `
...
`;
const networkParticlesTemplateHtml: string = /*html */ `
    <canvas class="web-component__canvas"></canvas>
`;

networkParticlesTemplateElement.innerHTML = /*html */ `
  <style>
    /* Reset */
    ${WebComponentCssVariables}
    ${WebComponentCssReset}
    ${WebComponentCssDarkTheme}
    /* Actual style */
    ${networkParticlesCssStyle}
  </style>
  
  ${networkParticlesTemplateHtml}
`;
 ...
 * @extends HTMLElement
 */
class NetworkParticles extends HTMLElement {
  /**
   ...
   * @type {NetworkParticlesCreator}
   */
  effectHandler: NetworkParticlesCreator;
  ...
  /**
   * Creates a new instance of the `NetworkParticles` class.
   *
   * @constructor
   */
  ...
  constructor() {
    ...
    const clonedTemplate: Node =
      networkParticlesTemplateElement.content.cloneNode(true);
     ...
     * @type {NetworkParticlesCreator}
     */
    this.effectHandler = new NetworkParticlesCreator(this.canvas, 100);
    // this.resizeCanvas();
  }
  ...
  resetCanvasToMatchScreen(): void {
    ...
    this.effectHandler = new NetworkParticlesCreator(this.canvas, 150);
...
customElements.define("network-particles", NetworkParticles);
